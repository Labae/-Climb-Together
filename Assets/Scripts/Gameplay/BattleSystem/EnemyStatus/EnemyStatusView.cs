...
{
    public class EnemyStatusView : BaseView
    {
        ...
        protected override void ValidateComponents()
        {
            ...
            {
                GameLogger.Error(ZString.Format("[{0}] Enemy Name Text is not set!", name), LogCategory.UI);
            }
            if (_healthBarContainer == null)
            {
                GameLogger.Error(ZString.Format("[{0}] Health Bar Container is not set!", name), LogCategory.UI);
            }
            if (_healthBarFill == null)
            {
                GameLogger.Error(ZString.Format("[{0}] Health Bar Fill is not set!", name), LogCategory.UI);
            }
            if (_shieldContainer == null)
            {
                GameLogger.Error(ZString.Format("[{0}] Shield Container is not set!", name), LogCategory.UI);
            }
            if (_shieldIconPrefab == null)
            {
                GameLogger.Error(ZString.Format("[{0}] Shield Icon Prefab is not set!", name), LogCategory.UI);
            }
        }
